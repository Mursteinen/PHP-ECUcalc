...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    function loadSolutionData($solution) {
        $filePath = './351761_solutions/' . $solution . '.json';
    ...
    foreach ($selectedSolutions as $solution) {
        $solutionData = loadSolutionData($solution);
        if ($solutionData) {
    ...
    $destinationFile = 'uploads/' . $baseFileName . $suffix . '_NoCHK.bin';
    ...
    echo '<a href="download.php?file=' . urlencode($destinationFile) . '" class="download-button">Download the updated file</a>';
} else {
